...
Throughout this repo and `AutoEval`, we use the [sentinel value of `-1`](https://github.com/jacobphillips99/mse-check/blob/main/utils/server.py#L22) to indicate that a server is deployed on a cloud-based service that obscures the underlying port and removes it from the URL.
...
Note that GPU-based Modal servers have a longer spin-up time than CPU-based servers as they must download and cache model weights. It is reccomended to send a test request or to "warm" a pod before using the server in a performance-critical application such as `AutoEval`. We also provide a "keep alive" script to keep a pod warm for a given interval; see [`scripts/keep_alive.py`](https://github.com/jacobphillips99/mallet/blob/main/scripts/keep_alive.py) for more details.
