...
from vlm_autoeval_robot_benchmark.models.vlm import VLMInput, parse_vlm_response
...
def print_messages(messages: list[dict[str, Any]]) -> None:
    for message in messages:
        ...
        for chunk in content:
            print(chunk["text"] if chunk["type"] == "text" else chunk["image_url"])


def print_history(history: deque[VLMInput]) -> None:
    for vlm_input in history:
        print(vlm_input.prompt)
        print(vlm_input.images)
